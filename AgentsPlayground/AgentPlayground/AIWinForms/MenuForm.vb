...
Public Class MenuForm
    ...
    Private Function CreateMenuItemPanel(def As MenuItemDef) As Panel
        ...
        ' --- Draw glow border ON the panel itself (not as a child control) ---
        ' The glow must be painted on menuItemPanel's own Paint event so it 
        ' appears behind all other children and doesn't intercept mouse events.
        AddHandler menuItemPanel.Paint, Sub(s, e)
                                            Dim g As Graphics = e.Graphics
                                            g.SmoothingMode = SmoothingMode.AntiAlias
                                            Using pen As New Pen(def.GradientStart, 2.0F)
                                                pen.EndCap = LineCap.Round
                                                pen.StartCap = LineCap.Round
                                                g.DrawRectangle(pen, 0.5F, 0.5F, 559, 69)
                                            End Using
                                        End Sub
